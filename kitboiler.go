...
func (p Pkg) generateOptionSetters(name, typ string) []string {
	...
	if strings.HasPrefix(typ, "...") && strings.HasSuffix(typ,"Setter") {
		...
		_, spec, err := typeSpec(importPath, bareType, SrcDir)
...
// genStubs will panic.
func genStubs(iface string, fns []Func) []byte {
	var buf bytes.Buffer
	...
	svc := Service{Funcs: fns, IFace: ifaceName, Imports: importMap}
